// src/components/BodyTypeScanner.jsx
import React, { useState, useRef } from "react";
import axios from "axios";

export default function BodyTypeScanner({ onComplete }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${API}/analyze/body`, formData, {
        headers: {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#181e29] text-white p-4">
      <h1 className="text-4xl font-bold mb-12 text-center">Body Type Analysis</h1>
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded shadow-inner">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-lg font-bold capitalize">{result.body_type.replace(/_/g, " ")}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Confidence</p>
              <div className="flex items-center">
                <span className={`inline-block w-4 h-4 rounded-full ${getConfidenceColor(result.confidence)} mr-2`}></span>
          ...
          <details className="mt-4">
            <summary className="cursor-pointer text-sm text-blue-600">Show extracted features</summary>
            <ul className="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs text-gray-600">
              {result.features.map((f, i) => (
                <li key={i}>Feature {i + 1}: {f.toFixed(4)}</li>
              ))}
            </ul>
          </details>
        </div>
